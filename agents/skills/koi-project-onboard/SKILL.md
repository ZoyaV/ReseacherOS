...
description: >-
  ...
  interview: code scan, light literature framing, then stepwise dialogue on
  problem → causes/hypotheses → methods (several questions per layer to learn
  the researcher's view), koi-prose-style on all UI text, then write
  koi-structure/project.md. TRIGGER when the user asks to «подключить проект»,
  onboard, attach a repo to ResearchOS, or points at a sibling folder that has
  code but no koi-structure/.
---
...
→ 6 Финальный prose-pass на skeleton → 6b Write (только после «пиши») → handoff
```
...
updated: <ISO-8601 Z>
format: koi/1
git_repo: <true если есть <repo>/.git, иначе false или опустить>
# git_sync_branch: koi/research   # только если git_repo: true и команда так договорилась
...
- [ ] `git_repo` согласован с наличием `.git`

После записи кратко скажи: путь к `project.md`, `id` проекта, что сделать в UI
(обновить discovery / открыть карту), следующий шаг.
...
| Закоммитить `koi-structure/` | **koi-project-sync** (если `git_repo: true`) |
...
- Словарь узлов: `docs/human/getting-started.md` (§3–§5)
