...
class CursorAppTest(unittest.TestCase):
    def test_cursor_is_running_macos(self) -> None:
        with patch("koi.cursor.app.platform.system", return_value="Darwin"):
            with patch(
                "koi.cursor.app._run_command",
                return_value=type("P", (), {"returncode": 0, "stdout": "", "stderr": ""})(),
            ):
                self.assertTrue(cursor_is_running())

    def test_cursor_is_frontmost(self) -> None:
        with patch("koi.cursor.app.cursor_frontmost_app_name", return_value="Cursor"):
            self.assertTrue(cursor_is_frontmost())
        with patch("koi.cursor.app.cursor_frontmost_app_name", return_value="Safari"):
            self.assertFalse(cursor_is_frontmost())

    def test_cursor_is_active_uses_frontmost(self) -> None:
        with patch("koi.cursor.app.cursor_is_frontmost", return_value=True):
            self.assertTrue(cursor_is_active())
        with patch("koi.cursor.app.cursor_is_frontmost", return_value=False):
