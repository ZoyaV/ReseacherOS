#!/usr/bin/env python3
"""ResearchOS Paper Inbox — watcher + queue status for NeurIPS paper generation."""
...
def main() -> None:
    ...
    p_watch = sub.add_parser(
        "watch",
        help="Watch paper queue; print PAPER_WAKE lines",
    )
    p_watch.set_defaults(func=lambda _a: run_watch())

    p_pending = sub.add_parser("pending", help="Show pending Paper tasks")
    ...
    p_boot = sub.add_parser(
        "bootstrap",
        help="Print first message for ResearchOS Paper Inbox chat",
    )
