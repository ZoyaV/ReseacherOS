...
| Задача | Как |
|--------|-----|
| Подключить существующий code-репо к ResearchOS | скилл **koi-project-onboard** — code scan → lit framing → сократовский диалог (problem → cause/hyp → method); **каждое** сообщение человеку по **koi-prose-style** → запись `koi-structure/project.md` |
